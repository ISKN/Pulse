 ...
 Halfedge * VoronoiDiagramGenerator::ELleftbnd( Point *p)
{
	...
	he = ELgethash(bucket);
	if(he == ( Halfedge *) NULL)			//if the HE isn't found, search backwards and forwards in the hash map for the first non-null entry
	{   
		for(i=1; 1 ; i += 1)
		{	
...
{
	if(he -> ELedge == ( Edge *)NULL) //if this halfedge has no edge, return the bottom site (whatever that is)
		return(bottomsite);
 ...
 Edge * VoronoiDiagramGenerator::bisect( Site *s1,	Site *s2)
{
	...
	ref(s1); 
	ref(s2);
	newedge -> ep[0] = ( Site *) NULL; //to begin with, there are no endpoints on the bisector - it goes to infinity
...
float VoronoiDiagramGenerator::dist( Site *s, Site *t)
{
	float dx,dy;
...
void VoronoiDiagramGenerator::PQinsert( Halfedge *he, Site * v, float offset)
{
	 Halfedge *last, *next;
